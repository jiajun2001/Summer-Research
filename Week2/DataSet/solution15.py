...
def q1_bfs(node1, node2, graph):
    ...
    # INSERT YOUR CODE BELOW
    # ~ 8 lines
    second_degree = q1_get_neighbor(node1, graph)
    for i in second_degree:
        if node2 == i:
            return True
    
    for j in second_degree:
        third_degree = q1_get_neighbor(j, graph)
    if node2 in third_degree:
        return True
    else:
        return False
...
def q2_dijkstra(node1, node2, graph):
    ...
    while True:
        ...
        for n in costs:
            ...
            # Assign costs values to a variable
            # if that node is not in visited
            # then check whether that variable is less than max_c
            # then update the node and update the max_c value
