...
def brute_force_set(li):
    ...
    # ~ 8 lines
    # find minimal number of sets that cover all 1 to 10
    minimum = []
    for i in range(2, len(li) + 1):
        for j in itertools.combinations(li, i):
            ...
            for k in j:
                for l in k:
                    covered[l] = True
            if len(covered) == 10 and len(minimum) == 0:
                minimum = j
            elif len(covered) == 10 and (len(minimum) > len(j)):
                minimum = j
...
def greedy_set(list_sets):
    ...
    while True:
        ...
        for iter_set in sets:
            current_uncovered = 0
            for iter_int in iter_set:
                if iter_int not in covered:
                    current_uncovered += 1
            # print(current_uncovered)
            if current_uncovered > max_uncover:
                max_uncover = current_uncovered
                max_set = iter_set
                # print(max_set)
        ...
        # mark covered integers
        for iter_int in max_set:
            covered[iter_int] = True
        sets.remove(max_set)
    ...
    for i in range(num_row):
        for j in range(num_col+1):
            if j < items[i][1]:
                ...
                grid[i][j] = max(grid[i-1][j], grid[i-1][j-items[i][1]] + items[i][2])
