...
def q1_bfs(node1, node2, graph):
    ...
    # ~ 8 lines
    list1 = []
    for i in q1_get_neighbor(node1,graph):
        list1.append(i)
        if len(q1_get_neighbor(i,graph)) > 0:
            for n in q1_get_neighbor(i,graph):
                list1.append(n)
    if node2 in list1:
        return True
    else:
...
def q2_dijkstra(node1, node2, graph):
    ...
    while True:
        ...
        for n in costs.values():
            list1.append(n)
        min_cost = min(list1)
        if costs[min_cost] not in visited:
            node = costs[min_cost]
        ...
        # ~ 6 lines
        # INSERT YOUR CODE BELOW
        neighbors = q2_get_neighbor(node, graph)
