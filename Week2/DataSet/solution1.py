...
def add_element(val):
    ...
    global collision
    hash_number = hash4_b(val)
    if table[hash_number]: # condition is True if list is not empty, else False
        collision += 1     # this if block must be before insertion to get accurate result
    table[hash_number].insert(0, val)
