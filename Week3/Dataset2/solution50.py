...
def brute_force_set(li):
    ...
    for i in range(1,len(li)+1):
        lis.append(i)
        newli = list(itertools.combinations(li,i))
        lis.append(newli)
    return lis
...
def greedy_set(list_sets):
    ...
    while True:
        ...
        max_set = sets[max_len_idx].copy()
        ans.append(list_sets[max_len_idx])
        
        for i in range(len(sets)):
            sets[i] = list(set(sets[i]) - set(max_set))
        for item in max_set:
            covered[item] = 1
            if len(covered) == 10:
                return ans
...
def knapsack_dp(bag_size, items):
    ...
    for i in range(1, num_row):
        for j in range(num_col+1):
            grid[i][j] = grid[i-1][j]
            if j >= items[i][1]:
                grid[i][j] = max(grid[i][j],grid[i-1][j-items[i][1]] + items[i][2])
    return grid
