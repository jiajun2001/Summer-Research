...
# %%
def hash1(val, n): #val = 'adelaide'
    return 1 #always returns 1

def hash2(val, n):
    return len(val) #returns the length of value
...
def hash4(val, n):
    ...
    ph = 0 #sum of all
    for i in val: #looping through 'adelaide'
        if i in d: #if letter is in d
            ph += d[i] #searches through the index and adds it to the sum of ph
    return ph % 10 #return placeholder % 10
...
def add_element(val):
    ...
    j = hash4_b(val) #hashes val
    global collision
    if len(table[j]) > 0: #if there is an item in (the spot) j
        collision += 1 #adds 1 to collision
    table[j].append(val) #appends value to j
...
def search(val):
    ...
    # INSERT YOUR CODE BELOW
    k = hash4_b(val)
    if val in table[k]: #if value is in table (at spot) k
        return True
    else:
        return False
        
def hash4_b(val): #hash function
    d = {'a': 1, 'b': 2, 'c': 3, 'd': 5, 'e': 7, 'f': 11, 'g': 13, 'h': 17, 'i': 19, 'j': 23, 'k': 29, 'l': 31, 'm': 37, 'n': 41, 'o': 43, 'p': 47, 'q': 53, 'r': 59, 's': 61, 't': 67, 'u': 71, 'v': 73, 'w': 79, 'x': 83, 'y': 89, 'z': 97}
    ...
    # INSERT YOUR CODE BELOW
    ph = 0 #sum of all
    for i in val: #looping through val
        if i in d: #if letter is in d
            ph += d[i] #searches through the index and adds it to the sum of ph
        else: #else adds 0/changes nothing
            ph += 0
    return ph % size #return placeholder % size
# ----------------
# TEST CODE BELOW
...
def check_dup(li):
    ...
    for i in li: #for i in li
        if i not in ph: #if i is not in ph
            ph.append(i) #only appends one value at a time
    if len(li) == len(ph): # if the length of li is the same as length of ph, return false
        return False
...
def two_sum(li, target):
    ...
    for i in range(len(li)):
        pear = target - li[i] #if target = pear + li[i] and pear number is in dict
        if pear in ph: #if pear in ph dictionary
            return (i, li.index(pear)) #return the two indexes
        ph[li[i]] = li[i] #adds the number we need to dictionary
