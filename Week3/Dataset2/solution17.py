...
def hash4(val, n):
    d = {'a': 1, 'b': 2, 'c': 3, 'd': 5, 'e': 7, 'f': 11, 'g': 13, 'h': 17, 'i': 19, 'j': 23, 'k': 29, 'l': 31, 'm': 37,
         'n': 41, 'o': 43, 'p': 47, 'q': 53, 'r': 59, 's': 61, 't': 67, 'u': 71, 'v': 73, 'w': 79, 'x': 83, 'y': 89,
         'z': 97}
    num = sum([d[i] for i in val])
    print(num % n)
    return num % n
...
def add_element(val):
    ...
    table[hash4_b(val)].append(val)
    if len(table[hash4_b(val)]) >= 2:
        collision += 1
...
def hash4_b(val):
    ...
    # INSERT YOUR CODE BELOW
    global size
    num = sum([d[i] for i in val if i in d.keys()])
    return num % size
# ----------------
# TEST CODE BELOW
...
def two_sum(li, target):
    ...
    for i in range(len(li)):
        start = li[i]
        next = i + 1
        for j in range(next, len(li)):
            if start + li[j] == target:
                return [j, i]
