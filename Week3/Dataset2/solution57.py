...
def brute_force_set(li):
    ...
    for i in range(1, len(li)):
        combinations = itertools.combinations(li, i)
...
def knapsack_dp(bag_size, items):
    ...
    for i in range(num_row):  # 这俩循环的取值为行与列，每件物品进行循环
        for j in range(1, bag_size + 1):  # 从轻的开始拿
            if weight[i] <= j:
                grid[i][j] = max(grid[i - 1][j - weight[i]] + price[i], grid[i - 1][j])
            else:
                grid[i][j] = grid[i - 1][j]
    return grid
