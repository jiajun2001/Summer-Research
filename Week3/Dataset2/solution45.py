...
def greedy_set(list_sets):
    ...
    while True:
        ...
        # ~ 15 lines
        uncovered_integers = set(range(1, 11)) - set(covered)
        uncovered_sizes = [len(uncovered_integers & set(item)) for item in sets]
        most_index = uncovered_sizes.index(max(uncovered_sizes))
        moset_uncovered_set = sets[most_index]
        ans.append(moset_uncovered_set)
        covered = set(covered).union(set(moset_uncovered_set))
        del sets[most_index]
    ...
    for i in range(0,num_row):
        for j in range(1,num_col+1):
            if j < items[i][1]:
                ...
                grid[i][j]=max(grid[i-1][j-items[i][1]]+items[i][2],grid[i-1][j])
    return grid
